...
export function VoteButton({ postId, initialCount, initialHasVoted = false, size = "sm", onVoteChange }: VoteButtonProps) {
  const { user } = useAuth();
  ...
  const handleVote = async (e: React.MouseEvent) => {
    ...
    try {
      const res = newHasVoted
        ? await feedbackService.vote(postId, user.id)
        : await feedbackService.unvote(postId, user.id);
