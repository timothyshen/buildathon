...
function CommentItem({ comment, postId, isReply = false, onCommentAdded }: CommentItemProps) {
  const { user } = useAuth();
  ...
  const handleReply = async () => {
    ...
    const res = await feedbackService.addComment(postId, user.id, replyText.trim(), comment.id);
    if (res.success) {
      setReplyText("");
      setShowReply(false);
      onCommentAdded();
  ...
  const handleEdit = async () => {
    ...
    setIsSubmitting(true);
    const res = await feedbackService.updateComment(comment.id, editText.trim());
    if (res.success) {
      setIsEditing(false);
      onCommentAdded();
    } else {
  ...
  const handleDelete = async () => {
    setIsSubmitting(true);
    const res = await feedbackService.deleteComment(comment.id);
    if (res.success) {
      onCommentAdded();
    } else {
...
export function CommentSection({ postId, comments, onRefresh }: CommentSectionProps) {
  const { user } = useAuth();
  ...
  const handleSubmit = async () => {
    ...
    setIsSubmitting(true);
    const res = await feedbackService.addComment(postId, user.id, newComment.trim());
    if (res.success) {
      setNewComment("");
      onRefresh();
    } else {
        ...
        {user ? (
          ...
          <div className="rounded-lg border border-dashed p-4 text-center">
            <p className="text-sm text-muted-foreground">
              <button onClick={() => router.push("/login?redirect=/feedback")} className="text-primary hover:underline">
                Sign in
              </button>{" "}
              to leave a comment.
            </p>
